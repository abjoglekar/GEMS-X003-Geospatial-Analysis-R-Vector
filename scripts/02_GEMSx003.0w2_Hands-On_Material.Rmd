...
For those of you familiar with the `dplyr` package, you can also use the `group_by()`
and `summarize()` functions to aggregate variable(s) of interest across groupings.

```{r dplyr_agg}
library(dplyr)
world_agg2 = world |>
  group_by(continent) |>
  summarize(pop = sum(pop, na.rm = TRUE), area = sum(area_km2))
world_agg2
```

This approach has more flexibility, readability, and control over the new column names, 
but does require learning the `tidyverse` syntax. 
...
tm_shape(world) + tm_polygons(col = "white") +
  ...
  tm_credits("Source: International Coffee Organization.") +
  tm_layout(main.title = "World coffee production (thousand 60-kg bags) by country, 2017", main.title.size = 1,
            bg.color = "lightblue")
```
...
```
sf2 <- merge(sf1, dat1, by.x = "var1", by.y = "var2", all = FALSE)
```

-   Left (outer) join preserves features from the first data set

```
sf2 <- merge(sf1, dat1, by.x = "var1", by.y = "var2", all.x = TRUE)
```

-   Right (outer) join preserves features from the second data set [not generally applicable for spatial data]

```
sf2 <- merge(sf1, dat1, by.x = "var1", by.y = "var2", all.y = TRUE)
```

-   Outer join preserves all features from both data sets [not generally applicable for spatial data]

```
sf2 <- merge(sf1, dat1, by.x = "var1", by.y = "var2", all = TRUE)
```

By default, `merge()` performs an inner join: `merge(x, y, by, all = FALSE)`. 

```{r attr_mrg2}
world_coffeeI = merge(world, coffee_data, by = "name_long")
nrow(world_coffeeI)
```

But there were 47 observations in our `world_coffee` dataset! We can identify the rows that did not match using the `setdiff()` function as follows:
```{r setdiff}
setdiff(coffee_data$name_long, world$name_long)
```

The result shows that `name_long` equaling `Congo, Dem. Rep. of` or `Others` is not present in the world dataset. Often country names are abbreviated and easily missed. You can reclassify the DRC name in (either) dataset and remerge. Identifying the spelling of the DRC country name in the world dataset can either be done manually or via a regular expression. 

```{r regex}
grep("Dem*.+Congo", world$name_long, value = TRUE)
```
**Note:** The object order matters when merging `sf` and `data.frame` objects. 

- If you list the `sf` object first, the result will be an `sf` object.

- If you list the `data.frame` object first, the result will be a `data.frame` object. 
...
3.  Find and plot the segments of the Mississippi river that run through groundwater province 1
